...
// Local Imports
import org.xj3d.ui.swt.widgets.*;
...
public class BrowserView extends ViewPart
    implements BrowserComponent, BrowserCoreListener, ShellListener, DisposeListener,
KeyListener, IPartListener2 {
    ...
    private static final String PROPERTY_SETUP_ERR =
        "Error setting up system properties";
    ...
    private static final String SHADED_RENDERING_MODE_MSG =
        "Shaded rendering mode disabled";
    ...
    *  Equivalent to 1 frame per second */
    ...
    /** The workbench window part service */
    ...
    public void createPartControl(Composite parent) {
        ...
        BrowserConfig parameters = new BrowserConfig( new HashMap( ) );
        parameters.showOpenButton = true;
        parameters.showReloadButton = true;
        parameters.showStatusBar = true;
        parameters.showFPS = true;
        ...
        if ( (parameters.showDash && parameters.dashTop) || (parameters.showUrl && parameters.urlTop)
            ...
            // we've preserved the arguments that come from the awt factory
            // impl,
            // but they are prone to ambiguity about where the toolbars
            // go - so we'll sort that out immediately
            if ((parameters.showDash && parameters.showUrl) && (parameters.dashTop == parameters.urlTop)) {
        ...
        surfaceComposite = new Composite(composite, SWT.NONE);
        
        surfaceComposite.setLayout(new FillLayout());
        
        SimpleSWTSurface surface = new SimpleSWTSurface(
            surfaceComposite,
            SWT.NONE,
            caps);
        ...
        cursorManager = new CursorManager(glCanvas, skinProperties, messageBuffer);
        ...
        WorldLoaderManager worldLoader = mainCanvas.getWorldLoaderManager();
        
        viewpoint_manager = mainCanvas.getViewpointManager();
        
        ScriptManager sm = mainCanvas.getScriptManager();
        ScriptLoader scriptLoader = sm.getScriptLoader();
        
        ScriptEngine jsai = new VRML97ScriptEngine(
            browserCore,
            route_manager,
            state_manager,
            worldLoader);
        jsai.setErrorReporter(messageBuffer);
        
        ScriptEngine ecma = new JavascriptScriptEngine(
            browserCore,
            route_manager,
            state_manager,
            worldLoader);
        ...
        ScriptEngine ecma_sai = new ECMAScriptEngine(
            browserCore,
            viewpoint_manager,
            route_manager,
            state_manager,
            worldLoader);
        ecma_sai.setErrorReporter(messageBuffer);
        ...
        setupProperties(browserCore, worldLoader, parameters.textureQuality);
        ...
        if (parameters.showDash) {
            ...
            if (parameters.dashTop) {
                ...
                dashComposite = new Composite(composite, SWT.NONE);
        ...
        IActionBars iab = getViewSite().getActionBars();
        IStatusLineManager slm = iab.getStatusLineManager();
        slm.add( new FrameRateContributionItem(
            BrowserViewConstants.FRAME_RATE_CONTRIBUTION_ITEM_ID,
            browserCore));
        iab.updateActionBars();
        
        loadProgressListener = new LoadProgressListener(
            parent.getDisplay(),
            slm,
            messageBuffer);
        
        ResourceConnection.addGlobalProgressListener(loadProgressListener);
        worldLoader.setProgressListener(loadProgressListener);
    ...
    public void keyReleased( KeyEvent ke ) {
        ...
        case SWT.NONE:
            switch( keyCode ) {
            case SWT.PAGE_DOWN:
                viewpoint_manager.nextViewpoint( );
                break;
                
            case SWT.PAGE_UP:
                viewpoint_manager.previousViewpoint( );
                break;
                
            case SWT.HOME:
                viewpoint_manager.firstViewpoint( );
                break;
                
            case SWT.END:
                viewpoint_manager.lastViewpoint( );
                break;
            }
            break;
        ...
        case SWT.ALT:
            if ( keyCode == A_KEY ) {
                ...
                */
                ...
                if ( pointrender ) {
                    statusBar.setStatusText( POINT_RENDERING_MODE_MSG );
                    browserCore.setRenderingStyle( Xj3DConstants.RENDER_POINTS );
                } else if( wireframe ) {
                    statusBar.setStatusText( WIREFRAME_RENDERING_MODE_MSG );
                    browserCore.setRenderingStyle( Xj3DConstants.RENDER_LINES );
                } else {
                    statusBar.setStatusText( SHADED_RENDERING_MODE_MSG );
    ...
    /**
     * The shell has changed from a minimized to a normal state. Restore the
     * frame rate to normal.
     *
     * @param evt The shell event that caused the method to be called.
     */
    ...
    /**
     * The shell has changed from a normal state to minimized. Throttle back the
     * frame rate.
     *
    ...
    /**
     * We're being disposed of. Notify the browser's Canvas to clean up in
     * anticipation of it's dispose method being called through the normal swt
     * chain of destruction. Notify the ImageLoader to dispose of it's cached
     * images.
     */
    ...
    /**
     * Called to instruct the component instance to destroy itself and any used
     * resources. It will not be used again.
     */
    public void destroy() {
        if (browserCore != null) {
    ...
    /**
     * Ignored. Notification that the browser is shutting down the current
     * content.
     */
    ...
    /**
     * The browser has been disposed by the user calling the dispose method on
     * the ExternalBrowser instance. Release our reference to the browser core
     * so that when this control is disposed, we don't bother with releasing
     * browser resources.
     */
     ...
     * registering all the properties needed for content and protocol handlers
     * used by the URI system. Only needs to be run once at startup.
     *
    ...
    private void setupProperties(final BrowserCore core,
        final WorldLoaderManager wlm, final String textureQuality) {
