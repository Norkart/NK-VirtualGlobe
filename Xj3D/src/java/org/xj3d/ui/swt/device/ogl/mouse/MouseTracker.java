...
/**
 * A tracker implementation for mouse devices associated
 * with instances of SWT UI widgets using an OpenGL renderer.
 *
 * @author Rex Melton
 * @version $Revision: 1.9 $
 */
public class MouseTracker extends Tracker
    implements MouseListener, MouseMoveListener, Listener {
    ...
    public MouseTracker(GraphicsOutputDevice surface, String id) {
        ...
        tstate[0].buttonMode[1] = ButtonModeConstants.NOTHING;
        tstate[0].pickingEnabled[1] = false;
        tstate[0].buttonMode[2] = ButtonModeConstants.NAV1;
        ...
        tstate[1].buttonMode[1] = ButtonModeConstants.NOTHING;
        tstate[1].pickingEnabled[1] = false;
        tstate[1].buttonMode[2] = ButtonModeConstants.NAV1;
    ...
    public void mouseDown(MouseEvent evt) {
        // the awt class that was the ancestor of this did all
        // the buttonState setting in the mousePressed() method.
        // this implementation (setting on a press, clearing on
        // a release) seems more efficient and a more timely
        // setting of the state. but, if there are subtle state
        // machine implications elsewhere.........
        switch( evt.button ) {
            case 1:
    ...
    public void mouseMove(MouseEvent evt) {
        if((evt.stateMask & SWT.BUTTON_MASK) != 0) {
            // equivalent to a MouseDragged event type
            if(tstate[currentIndex].actionType == TrackerState.TYPE_NONE ||
            ...
            // equivalent to a MouseMoved event type
            if(tstate[currentIndex].actionType == TrackerState.TYPE_NONE ||
    ...
    public void handleEvent(Event event) {
        switch (event.type) {
            case SWT.MouseWheel:
                tstate[currentIndex].wheelClicks = -(event.count/3);
    ...
    private void updateMouseDetails(MouseEvent evt) {
        ...
        // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        // note: decrementing the control size so it
        // and the event position both zero relative.
        float height = point.y - 1;
        float width = point.x - 1;
        // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        lastMouseX = evt.x;
        lastMouseY = evt.y;
