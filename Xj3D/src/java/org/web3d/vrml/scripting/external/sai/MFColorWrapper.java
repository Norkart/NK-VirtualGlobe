...
class MFColorWrapper extends BaseFieldWrapper
    ...
    public void get1Value(int index, float[] col) {
       if (storedOutput)
            ...
            if (index<0 || index>=value.numElements)
                ...
                System.arraycopy(value.floatArrayValue,index*3,col,0,3);
        }
    }
    ...
    public void getValue(float[] col) {
        if (storedOutput)
            ...
            if (data.numElements!=0)
                System.arraycopy(data.floatArrayValue,0,col,0,data.numElements*3);
        }
    }
    ...
    public void getValue(float[][] col) {
        if (storedOutput)
            ...
            checkReadAccess();
            VRMLFieldData data=theNode.getFieldValue(fieldIndex);
            ArrayUtils.raise3(data.floatArrayValue,data.numElements,col);
        }
    }
