...
public class GeometryUtils {
    ...
    public boolean generateTriangleArrays(int changeFlags, boolean genTexCoords,
        ...
        this.vfCoord = vfCoord;
        this.vfColor = vfColor;
        this.vfNormal = vfNormal;
        this.vfTexCoord = vfTexCoord;
        ...
        if(((changeFlags & BaseComponentGeometryNode.COORDS_CHANGED) != 0) || initialBuild) {
        ...
        if(((changeFlags & BaseComponentGeometryNode.COORDS_INDEX_CHANGED) != 0) || initialBuild) {
            ...
            int max_coords = vfCoord.getNumPoints();
...
*/
        ...
        if(((changeFlags & BaseComponentGeometryNode.TEXCOORDS_CHANGED) != 0) || initialBuild) {
            if(vfTexCoord == null) {
                ...
                // know about more.
                int numTexSets = vfTexCoord.getNumSets();
        ...
        if(((changeFlags & BaseComponentGeometryNode.COLORS_CHANGED) != 0) ||
            (geom_impl_changed && vfColor != null)) {
...
*/
     ...
     */
    private void rebuildFaceLists(boolean required) {
        int i;
        ...
        if(vfConvex)
            buildConvexPolygons(required);
        else
            buildConcavePolygons(required);
...
*/
     ...
     * @param required True if we have to update everything
     */
    private void buildConvexPolygons(boolean required) {
     ...
     * @param required True if we have to update everything
     */
    private void buildConcavePolygons(boolean required) {
    ...
    private void updateTexCoordinateArray(int tset, int numTexSets) {
        ...
        // TODO: Do we need to support mixed component types
        if (vfTexCoord != null) {
        ...
        if(geomData.textureCoordinates == null || geomData.textureCoordinates.length < numTexSets) {
