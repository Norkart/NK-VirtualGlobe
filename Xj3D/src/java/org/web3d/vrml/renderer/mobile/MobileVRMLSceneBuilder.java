...
/**
 * A Mobile file loader implementation for reading VRML and X3D files and
 * building a Mobile scenegraph with them.
 * <p>
 *
 ...
 */
public class MobileVRMLSceneBuilder implements SceneBuilder {
    ...
    private MobileMainSceneBuilder mainSceneBuilder;

    /** Builder for proto content */
    private ProtoBuilder protoBuilder;

    /** Builder for proto content */
    private MobileExternProtoBuilder externProtoBuilder;
     ...
     * {@link org.web3d.vrml.renderer.mobile.nodes.MobileVRMLNode} interface as we
     ...
     */
    public MobileVRMLSceneBuilder(VRMLNodeFactory fac) {
        nodeFactory = (fac != null) ? fac : MobileNodeFactory.getMobileNodeFactory();

        HashMap defs = new HashMap();
        protoMap = new HashMap();
        externProtoMap = new HashMap();

        mainSceneBuilder = new MobileMainSceneBuilder(nodeFactory);
        ...
        externProtoBuilder = new MobileExternProtoBuilder(nodeFactory);
     ...
     *
     * @param clk The simulation clock to use for this run
     */
    public void reset() {
        isParsing = false;
    ...
    public void startDocument(String uri,
        ...
        mainSceneBuilder.startDocument(url, encoding, type, version, comment);
        protoBuilder.startDocument(url, encoding, type, version, comment);
        externProtoBuilder.startDocument(url, encoding, type, version, comment);
    ...
    public void startExternProtoDecl(String name) throws SAVException, VRMLException {
        if(protoNestings != 0) {
            ...
            //we are not nested so build a Mobile specific one
