 ...
 * <p>
 *
 * The base group type is a BranchGroup so that we can add and remove this
 * group from the parent. If the group node is DEF'd then the implementation
 * group changes to be a SharedGroup with the branch group added as a child.
 * Each sub-class should define their own group node that exists as a child of
 * this impl group and add it to this provided group node during the
 * <code>setupFinished()</code> method.
 * <p>
 * This class also does a lot of the handling of the named fields. While it
 * expects the subclass to initialise all the arrays, this class will handle
 * add and remove of children and bounds setting. This means that any time you
 * override one of the methods here, please be sure to also call this method.
 * <p>
 * Note that we <i>require</i> the implementation class to create their Group
 * node instance during the constructor. The children handling depends on the
 * existance of this node during the construction time so you must make sure
 * that it exists before we start using it.
 * <p>
 * This implementation will call {@link #setupFinished()} on children nodes
 * before it does its own processing.
 *
 * @author Justin Couch
 * @version $Revision: 1.25 $
 */
public abstract class J3DGroupingNode extends BaseGroupingNode
    ...
    /**
     * Construct a default instance of the grouping node type. Make sure that
     * you instantiate and assign {@link #implGroup} during the constructor.
     *
     * @param name The name of the type of node
     */
    protected J3DGroupingNode(String name) {
        super(name);
