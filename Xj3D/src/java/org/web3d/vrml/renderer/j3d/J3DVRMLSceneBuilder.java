 ...
 */
public class J3DVRMLSceneBuilder implements SceneBuilder {
    ...
    private J3DMainSceneBuilder mainSceneBuilder;

    /** Builder for proto content */
    private ProtoBuilder protoBuilder;

    /** Builder for proto content */
    private J3DExternProtoBuilder externProtoBuilder;
     ...
     * {@link org.web3d.vrml.renderer.j3d.nodes.J3DVRMLNode} interface as we
     ...
     */
    public J3DVRMLSceneBuilder(VRMLNodeFactory fac) {

        nodeFactory = (fac != null) ? fac : J3DNodeFactory.getJ3DNodeFactory();

        HashMap defs = new HashMap();
        protoMap = new HashMap();
        externProtoMap = new HashMap();

        mainSceneBuilder = new J3DMainSceneBuilder(nodeFactory);
        ...
        externProtoBuilder = new J3DExternProtoBuilder(nodeFactory);
    ...
    //----------------------------------------------------------
    // Methods required by the SceneBuilder interface.
    //----------------------------------------------------------
    ...
    //----------------------------------------------------------
    // Methods required by the ProtoHandler interface.
    //----------------------------------------------------------
    ...
    public void startExternProtoDecl(String name) throws SAVException, VRMLException {
        if(protoNestings != 0) {
            ...
            //we are not nested so build a J3D specific one
    ...
    //----------------------------------------------------------
    // Methods required by the ScriptHandler interface.
    //----------------------------------------------------------
    ...
    //----------------------------------------------------------
    // Methods required by the RouteHandler interface.
    //----------------------------------------------------------
    ...
    {
        mainSceneBuilder.setCapabilityOverrideMap(capBits, freqBits);
    }
    ...
    {
        mainSceneBuilder.setCapabilityRequiredMap(capBits, freqBits);
    }
}
