 ...
 */
public class TerrainDataSource extends AbstractTiledTerrainData
{
     ...
     */
    public TerrainDataSource(double[] data,
                                boolean mustCopy,
                                double[] stepDetails,
...
/*
        coord[2] = gridX * (float)gridStepX;
        coord[0] = height;
        coord[1] = -gridY * (float)gridStepY;
*/
        if(gridX >= 0 && gridY >= 0 && gridX < gridWidth && gridY < gridDepth)
        {
            float[] rgb = colorInterp.floatRGBValue(coord[1]);
    ...
    public float getHeightFromGrid(int gridX, int gridY)
    {
        ...
        return (float) heightMap[g_x + (g_y * gridWidth)];
    }
