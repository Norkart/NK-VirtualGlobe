...
public class IndexedTriangleFanArray extends IndexedVertexGeometry
{
    ...
    private static final String MIN_COUNT_MSG =
        "A fan with less than 3 vertices is specified at index ";

    /** Message when the total count is less than fan totals */
    private static final String TOTAL_COUNT_MSG =
        "The total requested indexes in the fan is greater than the " +
...
*/
        ...
        throws NotPickableException
    {
        ...
        for(int i = 0; i < numFans; i++)
        {
            ...
            {
                wkPolygon[3] = coordinates[indices[coord_offset] * 3];
                wkPolygon[4] = coordinates[indices[coord_offset] * 3 + 1];
                wkPolygon[5] = coordinates[indices[coord_offset] * 3 + 2];

                wkPolygon[6] = coordinates[indices[coord_offset + 1] * 3];
                wkPolygon[7] = coordinates[indices[coord_offset + 1] * 3 + 1];
                wkPolygon[8] = coordinates[indices[coord_offset + 1] * 3 + 2];
                coord_offset++;
