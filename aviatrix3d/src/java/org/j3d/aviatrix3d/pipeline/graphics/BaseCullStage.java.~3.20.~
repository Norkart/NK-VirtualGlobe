...
public abstract class BaseCullStage implements GraphicsCullStage
{
    ...
    protected static final String SHARED_VP_MSG =
    ...
    protected static final String SHARED_BG_MSG =
    ...
    {
        validSceneParents = new SceneGraphObject[numSurfaces][2];
        outputSceneParents = new SceneGraphObject[numSurfaces][2];
        ...
        layerCounts = new int[1];

        for(int i = 0; i < numSurfaces; i++)
            outputLayers[i][0] = new ViewportCollection();
                     ...
                     int numLayers)
    {
        ...
        if(outputLayers[0].length < numLayers)
        {
            outputLayers = new ViewportCollection[1][numLayers];
            for(int i = 0; i < numLayers; i++)
                outputLayers[0][i] = new ViewportCollection();
        ...
        if(checkOffscreens)
        {
            ...
            for(int i = 0; i < lastSubscene && !terminate; i++)
            {
                ...
                if(processedPBufferTextures.contains(pendingSubscenes[i]))
                {
                    validSceneParents[lastOutputList][0] =
                        (SceneGraphObject)pendingSubscenes[i];
                    validSceneParents[lastOutputList][1] =
                        (SceneGraphObject)pendingParents[i];
                }
                else
                {
                    ...
                    for(int j = 0; j < num_layers; j++)
                    {
                        ...
                        LayerCullable lc = pendingSubscenes[i].getCullableLayer(j);
                ...
                layerCounts[lastOutputLayer] = layer;
                lastOutputLayer++;
                lastOutputList++;
            }
            ...
            if(outputSceneParents.length < lastOutputList)
                outputSceneParents = new SceneGraphObject[lastOutputList][2];
                                ...
                                int layerId)
    {
        ...
        outputLayers[subsceneId][layerId].ensureCapacity(num_views);
        outputLayers[subsceneId][layerId].numViewports = 0;
        ...
        for(int i = 0; i < num_views; i++)
        {
            ...
            outputLayers[subsceneId][layerId].numViewports++;
        }
    }
                                ...
                                int viewIndex)
    {
        ...
        for(int i = 0; i < num_layers; i++)
        {
            ...
            if(layer.isMultipassViewport())
                cullMultipassViewportLayer(layer.getCullableScene(),
                                           subsceneId,
                                           layerId,
                                           viewIndex,
                                           i);
                                              ...
                                              int layerIndex)
    {
        ...
        c.ensureMultipassCapacity(1);
        c.multipass[0].mainScene.ensureCapacity(num_passes);
    ...
    protected void checkForOffscreens(ShapeRenderable shape)
    {
        ...
        for(int i =0; i < num_units; i++)
        {
            ...
            if(tex.isOffscreenSource())
            {
                ...
                if(!visitedNodes.contains(tex) && offscreen.isRepaintRequired())
                {
                    resizeOffscreenList();
                    pendingSubscenes[lastSubscene] = offscreen;
                    pendingParents[lastSubscene] = activeParent;
                    visitedNodes.add(tex);
                    lastSubscene++;
                }
            }
/*
            else if(tex instanceof MultipassTextureSource)
            {
                // Fill in multipass offscreen texture
System.out.println("Null cull stage does not process multipass textures yet");
            }
*/
        }
    }
     ...
     * <p>
     *
     * This base implementation cleans up the outputLayers list. Derived
    ...
    protected void cleanupOldRefs()
    {
        ...
        {
            for(int j = 0; j < outputLayers[i].length; j++)
            {
                if(outputLayers[i][j] == null)
                    break;

                for(int k = 0; k < outputLayers[i][j].numViewports; k++)
                {
                    ViewportLayerCollection c = outputLayers[i][j].viewports[k];
    ...
    protected final void resizeOutputLists()
    {
        ...
        SceneGraphObject[][] tmp_obj = new SceneGraphObject[new_size][];

        System.arraycopy(validSceneParents, 0, tmp_obj, 0, old_size);

        for(int i = old_size; i < new_size; i++)
            tmp_obj[i] = new SceneGraphObject[2];
        ...
        System.arraycopy(outputLayers, 0, tmp_vc, 0, outputLayers.length);
        System.arraycopy(layerCounts, 0, tmp_lc, 0, layerCounts.length);

        for(int i = outputLayers.length; i < new_size; i++)
        {
