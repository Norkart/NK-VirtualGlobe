...
public class TransparencyDepthSortStage extends BaseSortStage
{
    ...
    private static final String INVALID_DEPTH_MSG =
        "The depthBits must be greater than or equal to 1";
     ...
     *     depth sorting to
     * @throws IllegalArgumentException Depth bits is zero or negative
     */
                            ...
                            int instrCount)
    {
        ...
        for(int i = 0; i < numNodes && !terminate; i++)
        {
            ...
            {
                System.out.println("Unknown renderable node in sorter " +
                                   nodes[i].renderable);
                continue;
            }
        ...
        switch(trans)
        {
            ...
            case 1:
                instr.renderOps[idx++] = RenderOp.START_ALPHATEST;

                idx = appendOutputShape(idx,
                                        unsortedTransparentList[0],
                                        instr);

                instr.renderOps[idx++] = RenderOp.STOP_ALPHATEST;
        ...
        for(int i = 0; i < trans && !terminate; i++)
        {
            ...
            Shape3D shape = (Shape3D)unsortedTransparentList[i].renderable;
            BoundingVolume bounds = shape.getBounds();
                              ...
                              int instrCount)
    {
        ...
        for(int i = 0; i < numNodes && !terminate; i++)
        {
            ...
            // may have alpha is passed off onto the transparent list.
            // Now process the node type.
            if(nodes[i].renderable instanceof Pixmap)
            {
                Pixmap shape = (Pixmap)nodes[i].renderable;

                Raster raster = shape.getRaster();
            ...
            {
                System.out.println("Non-shape node in sorter " +
                                   nodes[i].renderable);
                continue;
            }
                                 ...
                                 GraphicsInstructions instr)
    {
        ...
        else if(node.renderable instanceof Pixmap)
        {
            Pixmap shape = (Pixmap)node.renderable;
            Raster raster = shape.getRaster();
            ...
            instr.renderList[idx].renderable = raster;
