...
public class GenericCullStage extends BaseCullStage
{
                                     ...
                                     GraphicsEnvironmentData envData)
    {
        ...
        angularResolution = (float)view.getFieldOfView() / viewport[2];
    }
...
//            checkForOffscreens(root);
    ...
    private void updateFrustum(GraphicsEnvironmentData envData)
    {
        ...
        float left = (float)envData.viewFrustum[0];
        float right = (float)envData.viewFrustum[1];
        float bottom = (float)envData.viewFrustum[2];
        float top = (float)envData.viewFrustum[3];
        float nearval = (float)envData.viewFrustum[4];
        float farval = (float)envData.viewFrustum[5];
        float x, y, z, w;
        float a, b, c, d;
