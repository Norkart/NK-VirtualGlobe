    ...
    implements GeometryRenderable, DeletableRenderable
{
    ...
    private static final String INVALID_ATTRIB_SIZE_MSG =
        "Invalid attribute size. Must be 0 < size < 4";

    /**
     * Error message when not enough values provided in the user array
     * for 3 component colours.
     */
    private static final String COLOR_3_SIZE_ERR =
        "Color array too short for 3 component colour";

    /**
     * Error message when not enough values provided in the user array
     * for 4 component colours.
     */
    private static final String COLOR_4_SIZE_ERR =
        "Color array too short for 4 component colour";
...
// Don't do anything with the 4th coord for now.
    ...
    //---------------------------------------------------------------
    /**
     * Queue this object for freeing up GL resources
     */
               ...
               InvalidWriteTimingException
    {
        ...
        if(numValid * vtx_size > vertices.length)
            throw new IllegalArgumentException(
                "Vertex array not long enough (" + vertices.length + ") for " +
                "numValid * vertex_type " + (numValid * vtx_size));
     ...
     *
     * @param index The index of the coordinate to get
     * @param coord The array to copy the color value in to
     * @throws ArrayIndexOutOfBoundsException If the coord array is not big
     *    enough for the coordinate dimensions
     */
        ...
        if(texCoords != null)
        {
            switch(type)
            {
                ...
                case TEXTURE_COORDINATE_2:
                    if(texCoords.length < numRequiredCoords * 2)
                        throw new IllegalArgumentException("texCoord array too short " +
                                                       "for 2D texture coordiantes");
                    break;

                case TEXTURE_COORDINATE_3:
                    if(texCoords.length < numRequiredCoords * 3)
                        throw new IllegalArgumentException("texCoord array too short " +
                                                       "for 3D texture coordiantes");
                    break;

                case TEXTURE_COORDINATE_4:
                    if(texCoords.length < numRequiredCoords * 4)
                        throw new IllegalArgumentException("texCoord array too short " +
            ...
            for(int i = 0; i < numSets; i++)
            {
                if((texCoords[i] == null) || (texCoords[i].length == 0))
                    throw new IllegalArgumentException(
                        ...
                        "any texture coordinates defined but requires " +
                        numCoords);
                ...
                {
                    case TEXTURE_COORDINATE_1:
                        if(texCoords[i].length < numRequiredCoords)
                            throw new IllegalArgumentException(
                                "Texture coordinate set " + i + " does not have " +
                                "enough values for a 1D texture. It has " +
                                texCoords[i].length + " but requires " + numCoords);
                    ...
                    case TEXTURE_COORDINATE_2:
                        ...
                        if(texCoords[i].length < num_tex_coords)
                            throw new IllegalArgumentException(
                                "Texture coordinate set " + i + " does not have " +
                                "enough values for a 2D texture. It has " +
                                texCoords[i].length + " but requires " + num_tex_coords);
                    ...
                    case TEXTURE_COORDINATE_3:
                        ...
                        if(texCoords[i].length < num_tex_coords)
                            throw new IllegalArgumentException(
                                "Texture coordinate set " + i + " does not have " +
                                "enough values for a 3D texture. It has " +
                                texCoords[i].length + " but requires " + num_tex_coords);
                    ...
                    case TEXTURE_COORDINATE_4:
                        ...
                        if(texCoords[i].length < num_tex_coords)
                            throw new IllegalArgumentException(
                                "Texture coordinate set " + i + " does not have " +
                                "enough values for a 4D texture. It has " +
                                texCoords[i].length + " but requires " + num_tex_coords);
        ...
        throws IllegalArgumentException, InvalidWriteTimingException
    {
        ...
        if((attribs != null) && (attribs.length < numRequiredCoords * size))
    ...
    protected int fillBufferData(GL gl)
    {
        int vtx_size = vertexFormat & 0x07;
        int offset = 0;
        gl.glBufferSubData(GL.GL_ARRAY_BUFFER, offset, numRequiredCoords * vtx_size * 4, vertexBuffer);
        offset += numRequiredCoords * vtx_size * 4;

        if((vertexFormat & NORMALS) != 0)
        {
            gl.glBufferSubData(GL.GL_ARRAY_BUFFER, offset, numRequiredCoords * 3 * 4, normalBuffer);
        ...
        if((vertexFormat & TEXTURE_MASK) != 0)
        {
            ...
            if(hasMultiTextureAPI && numTextureSets > 1)
            {
                for(int i = 0; i < numRenderedTextureSets; i++)
                {
                    ...
                    gl.glBufferSubData(GL.GL_ARRAY_BUFFER, offset, size, textureBuffer[set_id]);
                    offset += size;
                }
            }
            else
            {
                ...
                gl.glBufferSubData(GL.GL_ARRAY_BUFFER, offset, size, textureBuffer[0]);
                offset += size;
           }
        }
        if((vertexFormat & COLOR_MASK) != 0 && (vertexFormat & COLOR_SINGLE) == 0)
        {
            ...
            int size = ((vertexFormat & COLOR_3) != 0) ? 3 : 4;
            gl.glBufferSubData(GL.GL_ARRAY_BUFFER, offset, numRequiredCoords * size * 4, colorBuffer);
        ...
        if((vertexFormat & COLOR2_MASK) != 0)
        {
            gl.glBufferSubData(GL.GL_ARRAY_BUFFER, offset, numRequiredCoords * 3 * 4, color2Buffer);
            offset += numRequiredCoords * 3 * 4;
        }
        if((vertexFormat & FOG_MASK) != 0)
        {
            gl.glBufferSubData(GL.GL_ARRAY_BUFFER, offset, numRequiredCoords * 4, fogBuffer);
        ...
        if((vertexFormat & ATTRIBS) != 0)
        {
            ...
            for(int i = 0; i < num_attribs; i++)
            {
                int id = attribIds[i];
                ShaderAttribValue val = (ShaderAttribValue)attributes.get(id);
                gl.glBufferSubData(GL.GL_ARRAY_BUFFER, offset, numRequiredCoords * 4 * val.size, val.data);
...
//System.out.println("glBufferData avail: " + gl.isFunctionAvailable("glBufferData"));
//System.out.println("glGenBuffers avail: " + gl.isFunctionAvailable("glGenBuffers"));
        ...
        if(vboAvailable && useVbo)
        {
            try {
                ...
                System.out.println("Failed in using VBO's.  Disabling usage");
                // VBO's failed, fall back
                vboAvailable = false;
            }
        }
