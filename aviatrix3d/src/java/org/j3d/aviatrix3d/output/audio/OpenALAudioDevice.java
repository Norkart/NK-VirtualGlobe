    ...
    implements AudioOutputDevice
{
    ...
    private static final String DISPOSE_FAILURE_MSG =
        "An error occurred during the audio surface dispose handling.";

    /** Message when there is an error during the draw processing */
    private static final String DRAW_FAILURE_MSG =
        "An error occurred during the audio surface draw loop handling";

    /** Message when there is an error during the surface initialisation. */
    private static final String INIT_FAILURE_MSG =
        "An error occurred during the audio surface initialisation proces";
    ...
    public boolean draw(ProfilingData profilingData)
    {
        ...
        if(!initComplete)
        {
            ...
            {
                errorReporter.errorReport(INIT_FAILURE_MSG, oae);
                ...
                errorReporter.errorReport(INIT_FAILURE_MSG, null);
                initFailed = true;
                return false;
            }
        }
        else
        {
            ...
            {
                errorReporter.errorReport(DRAW_FAILURE_MSG, oae);
            }
        }
    ...
    public void dispose()
    {
        ...
        if(initComplete)
        {
            ...
            {
                errorReporter.errorReport(DISPOSE_FAILURE_MSG, oal);
            }
        }
    }
