    ...
    implements GraphicsOutputDevice
{
    ...
    public boolean draw(ProfilingData profilingData)
    {
        ...
        if(count > 1)
        {
            for(int i = 0; i < count && !terminate && draw_continue; i++)
            {
                if(surfaces[i] != null)
                {
                    ...
                    draw_continue = rp.render((GraphicsProfilingData)profilingData);
                }
            }
        }
        ...
        if(!terminate && draw_continue)
            draw_continue = canvasRenderer.render((GraphicsProfilingData)profilingData);
    ...
    protected boolean initCanvas()
    {
        ...
        switch(status)
        {
            case GLContext.CONTEXT_NOT_CURRENT:
                ...
                System.out.println("Trying to make current again");
