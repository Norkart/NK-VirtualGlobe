        ...
        throws IllegalArgumentException, InvalidWriteTimingException
    {
        ...
        if(numValid * vtx_size > vertices.limit())
            throw new IllegalArgumentException(
                "Vertex array not long enough (" + vertices.limit() + ") for " +
        ...
        throws IllegalArgumentException, InvalidWriteTimingException
    {
        ...
        else
        {
            ...
            for(int i = 0; i < numSets; i++)
            {
                ...
                {
                    case TEXTURE_COORDINATE_1:
                        if(texCoords[i].limit() < numCoords)
                            throw new IllegalArgumentException(
                                "Texture coordinate set " + i + " does not have " +
                                "enough values for a 1D texture. It has " +
                                texCoords[i].limit() + " but requires " + numCoords);
                        break;

                    case TEXTURE_COORDINATE_2:
                        num_tex_coords = numCoords * 2;

                        if(texCoords[i].limit() < num_tex_coords)
                            throw new IllegalArgumentException(
                                "Texture coordinate set " + i + " does not have " +
                                "enough values for a 2D texture. It has " +
                                texCoords[i].limit() + " but requires " + num_tex_coords);
                        break;

                    case TEXTURE_COORDINATE_3:
                        num_tex_coords = numCoords * 3;
                        if(texCoords[i].limit() < num_tex_coords)
                            throw new IllegalArgumentException(
                                "Texture coordinate set " + i + " does not have " +
