    ...
    implements NodeUpdateListener, ShaderObjectParent
{
    ...
    private ArrayList bufferGeometry;

    /** Geometry instances that implement VertexGeometry */
    private ArrayList vertexGeometry;
    ...
    private ArrayList updatedChildAttribList;
    ...
    public void updateSkeleton()
    {
        ...
        if(size != 0)
        {
            ...
            {
                ShaderJoint joint = (ShaderJoint)updatedChildAttribList.get(i);
            ...
            for(int i = 0; i < size; i++)
                ((VertexGeometry)vertexGeometry.get(i)).dataChanged(this);

            size = bufferGeometry.size();
            for(int i = 0; i < size; i++)
                ((BufferGeometry)bufferGeometry.get(i)).dataChanged(this);
        }
    }
    ...
    {
        if(!updatedChildAttribList.contains(child))
            updatedChildAttribList.add(child);
    }
...
*/
    ...
    private int updateShader(Shape3D shape)
    {
        ...
        if(s_geom instanceof BufferGeometry)
            bufferGeometry.add(s_geom);
        else if(s_geom instanceof VertexGeometry)
            vertexGeometry.add(s_geom);
