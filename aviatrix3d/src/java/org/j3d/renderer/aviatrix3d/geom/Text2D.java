...
public class Text2D extends Geometry
{
    ...
    private static final String INVALID_SPACING_MSG =
        "The value for the spacing is not positive definite: ";

    /** Message when the user provides a size that is <= 0 */
    private static final String INVALID_SIZE_MSG =
        "The value for the size is not positive definite: ";

    /** Message when the user provides a dodfy justification flag */
    private static final String INVALID_JUSTIFY_MSG =
        "Invalid justification constant provided. Must be one of the " +
        "JUSTIFY_* values defined by this class";
...
//System.out.println("line [" + i + "] " + local_y + " " + sizeScale + " " + spacingScale);
    ...
    {
        if(updateHandler != null && !updateHandler.isBoundsWritePermitted(this))
        ...
        throws InvalidWriteTimingException, IllegalArgumentException
    {
        ...
        if(spacing <= 0)
            throw new IllegalArgumentException(INVALID_SPACING_MSG + spacing);
        ...
        throws InvalidWriteTimingException, IllegalArgumentException
    {
        ...
        if(size <= 0)
            throw new IllegalArgumentException(INVALID_SIZE_MSG + size);
